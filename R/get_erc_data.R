...
stat_to_pull<-c(86401,
                ...
                86403,
                86404,
                #86301,#first 5 big cypress, last 2 everglades, remove 86301 honeymoon raws because Jill said it was squirrely
                86702,
...
url<-paste0('https://famprod.nwcg.gov/wims/xsql/nfdrs.xsql?stn=',i,'&sig=&user=lbradshaw&type=&start=',day,"-",mnth,"-",yr,"&end=",day,"-",mnth,"-",yr,'&time=&priority=&fmodel=16Y&sort=asc&ndays=')
print(url)
...
#because the evergaldes fuel models were defaulting to F but wanted them to be N, can remove this and just grab first fuel model ec value when Ever is up to date
#erc_needclean<-ifelse(i %in% c(86404,86704),xml_lst$nfdrs[2]$row$ec,xml_lst$nfdrs$row$ec)
erc<-as.double(unlist(xml_lst$nfdrs$row$ec))
#erc<-as.double(unlist(erc_needclean)) #get the erc value
...
return(list(round(mean_erc,1),erc_vals))#return the average ERC

}


date="20220407" #format of date to pass in 
erc_info<-get_mean_erc(date) #run function


avg_erc<-unlist(erc_info[1])
